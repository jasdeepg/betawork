...
def init_db():
    g.db.execute("CREATE TABLE entries(id INT PRIMARY KEY, number INT, owner VARCHAR(20), timeDay INT, power INT)")
...
def addto_db():
    ...
    g.db.execute("INSERT INTO entries VALUES(1, 'Jim', 1, 9, 100)")
    g.db.execute("INSERT INTO entries VALUES(2, 'Jim', 1, 10, 1200)")
    g.db.execute("INSERT INTO entries VALUES(3, 'Jim', 1, 11, 287)")
    g.db.execute("INSERT INTO entries VALUES(4, 'Jim', 1, 12, 2300)")
    g.db.commit()
    
def makeDict():
    cur = g.db.execute('SELECT id, i, owner, timeDay, power FROM entries')
